                            ...
                            for h in range(hi,len(bas)):
                                for i in range(ii,len(bas)):
                                    for j in range(ji,len(bas)):
                                        for k in range(ki,len(bas)):
                                            t = "%s%s%s%s%s%s%s%s%s%s%s" % ( bas[a], bas[b], bas[c], bas[d], bas[e], bas[f], bas[g],bas[h],bas[i],bas[j],bas[k])
                                            cont = cont + 1
                                            cgeral = cgeral + 1
                                            if (cont ==100000):
                                                ntime = datetime.now()
                                                ...
                                                flog=open("log11.log","w")
                                                flog.writelines("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s, - %s, %s, keys \n" % (a,b,c,d,e,f,g,h,i,j,k,ttime,cgeral))
                                                flog.close()
                                                cont = 0                          
                                            print(t)
                                        ki = 0
