                        ...
                        for g in range(gi,len(bas)):
                            for h in range(hi,len(bas)):
                                for i in range(ii,len(bas)):
                                    t = "%s%s%s%s%s%s%s%s%s" % ( bas[a], bas[b], bas[c], bas[d], bas[e], bas[f], bas[g],bas[h],bas[i])
                                    cont = cont + 1
                                    cgeral = cgeral + 1
                                    if (cont ==100000):
                                        ntime = datetime.now()
                                        ...
                                        flog=open("log9.log","w")
                                        flog.writelines("%s,%s,%s,%s,%s,%s,%s,%s,%s, - %s, %s, keys \n" % (a,b,c,d,e,f,g,h,i,ttime,cgeral))
                                        flog.close()
                                        cont = 0
